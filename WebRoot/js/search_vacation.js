...
{
	var url = "";
	url += "/Student/SearchVacationServlet?search_type=" + $("#search_type").val();
	...
	$.post(url, null, function(rs)
	{
		...
		for ( var i = 0; i < rs.length; i++)
		{
			...
			// str += "<td>" + rs[i].isApprov + "</td>";
			str += "<td>" + rs[i].start + "</td>";
			str += "<td>" + rs[i].end + "</td>";
			str += "<td>";
			if(rs[i].isApprov==1){
				str += "<a href='/Student/EditVacationServlet?id=" + rs[i].id + "'>批准</a> / ";
			}
			str += "<a href='/Student/DeleteVacationServlet?id=" + rs[i].id + "'>删除</a></td>";
			str += "</tr>";
			$("#table").append(str);
		}
	}, "json");
...
{
	var url = "", rt = "";
	url += "/Student/GetVacationCountPageServlet?search_type=" + $("#search_type").val();
